...
def extract(data_dir, out_dir):
    ...
    # remove the intersection data between mini and trainval
    tmp_sample_files = []
    ...
    # travel all sample data generate from convert_radar_point.py and append to train_annos or tv_annos
    ...
    # generate for different radius
    for radius in radius_list:
        # dataset, set_annos: 'train', train_annos
        for data_set, set_annos in zip(sets, all_annos):
            ...
            pc_im_means = None
            pc_im_stds = None
            # 注意：这里的annos是从image_pc_annotations.json(列表)读取的一帧sample的所有标注(数量>=0)
            for annos in set_annos:
                if len(annos) > 0:
                    ...
                    # used for radar feature normalization
                    if num_item == 1:
...
if __name__ == '__main__':
    args = parse_args()
    # 从数据集中sample归一化参数提取出合并为雷达点云图像的统一归一化参数
