...
def map_point_cloud_to_image(point_sensor_token, camera_token, min_dist=1.0):
    ...
    # Fourth step: transform into the camera, (x, y, z) at camera 3D coordinate
    ...
    # we can use pc.points[0:2] / depth to get pixel position
    depths = pc.points[2, :]

    # get position(u, v) in pixel coordinate
    ...
    # filter points outside depth range
    mask = np.logical_and(mask, depths > min_dist)
    # filter points outside the width of picture
    mask = np.logical_and(mask, points[0, :] > 1)
    mask = np.logical_and(mask, points[0, :] < cam['width'] - 1)
    # filter points outside the height of picture
    mask = np.logical_and(mask, points[1, :] > 1)
    mask = np.logical_and(mask, points[1, :] < cam['height'] - 1)
    # [18, n]
    pc[:2, :] = points[:2, :]
    
    # filter by mask(bool index)
    pc = pc[:, mask]
...
def draw_pc_image(pc, save_path, radius, im_height=900, im_width=1600):
    ...
    for i in range(num_points):
        ...
        if (depth > _DISTANCE_RANGE[0]) and (depth < _DISTANCE_RANGE[1]):
            v = np.sqrt(vx ** 2 + vy ** 2)
            # only consider vel in centain range
    ...
    # save radar rendered image used for fusion
...
def get_pc_info(sample_data_token):
    ...
    if not os.path.isfile(pcd_path):
        ...
        # re-generate radar points file as: [3, n]
...
def run():
    ...
    # if do not use multi-task, uncomment following code and comment code above
